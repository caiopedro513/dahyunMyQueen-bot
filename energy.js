let energy = function(target, client, username, mssg){
    var cmdsplt;
    var tgtpers;
    if (mssg.indexOf("@") === -1){
        cmdsplt = mssg.split(' ');
    ...
    connection.query(`SELECT username FROM fightclubstats WHERE username = ?`, [tgtpers], function (error, results, fields) {
        if (results.length === 0){
            ...
            connection.query(`SELECT energy FROM fightclubstats WHERE username = ?`, [tgtpers], function(error, results, fields){
                client.say(target, `${tgtpers} has  ⚡${results[0].energy}`);
            })
        }
    })
}
